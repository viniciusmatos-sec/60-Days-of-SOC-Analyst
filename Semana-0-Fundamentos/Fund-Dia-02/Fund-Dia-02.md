...
# A Prática do Dia: Subnetting Mastery (Capítulo 1)

Para fixar toda essa teoria na prática, concluí com 100% de aproveitamento o Capítulo 1 ("The Masked Octet") do bloco The Foundations of Subnetting no curso Subnetting Mastery, que utiliza uma excelente analogia de um prédio comercial de 4 andares para explicar a divisão de redes. O foco inicial da lição foi no último octeto (um bloco de 8 bits de um endereço IPv4 de 32 bits), que nos fornece 256 valores possíveis. Para segmentar a rede do prédio, reduzir o tráfego de broadcast e limitar a propagação de ameaças, aprendi a "pegar emprestado" 2 bits da porção de hosts, criando exatamente 4 sub-redes distintas para mapear os andares. Matematicamente, dividimos os 256 valores do octeto pelas 4 sub-redes, resultando em 64 endereços para cada; como o primeiro endereço (Network ID) e o último (Broadcast Address) de uma rede são sempre reservados, sobram 62 endereços utilizáveis por andar, o que atende perfeitamente à demanda de 50 dispositivos de cada pavimento. Por fim, entendi como a máscara de sub-rede funciona como uma máscara de baile ou fita crepe de pintura, "mostrando" os bits da sub-rede (iniciando com 'uns') e "escondendo" os bits de hosts (o resto em 'zeros'), o que revela instantaneamente a qual andar do nosso "prédio" aquele dispositivo pertence.
